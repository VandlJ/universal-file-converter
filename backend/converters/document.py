...
class DocumentConverter(BaseConverter):
    async def convert(
        ...
        if output_format == "pdf":
            return await self._to_pdf(input_path, output_path, ext, options, on_progress)
        ...
        if ext in ("txt", "log", "ini", "cfg", "env", "css", "json", "xml", "yaml", "toml", "csv", "tsv"):
            result = await self._convert_plain(input_path, output_path, ext, output_format, options)
            if on_progress:
                await on_progress(90)
            return result

        # Use pandoc for rich format conversions
        result = await self._convert_with_pandoc(
            input_path, output_path, ext, output_format, md_formatting, on_progress
        )
        return result
